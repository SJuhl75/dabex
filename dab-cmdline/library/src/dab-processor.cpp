...
// https://www.cel.kit.edu/download/Braun_RadarCon2010_MLEOFDMRadar.pdf
// https://bibliotekanauki.pl/articles/1954573.pdf
static
void	startIdxInfo (int si) {
//	fprintf (stderr, "** DBG startIndex\t= %d\n", si);
}
...
double  	cLevel          = 0;
int     	cCount          = 0;
static float snr		= 0; // w/o static float
static int ccc       		= 0;
	...
	//my_mscHandler. start ();
	//fprintf(stderr, "DBG: dabProcessor::run T_F=%d\n",T_F);
	...
	//fprintf(stderr, "DBG: dabProcessor::run-2 i=%d\n",i);
...
//           fprintf(stderr, "DBG: CoarseFreqOff: %.1fHz\n", coarseOffset);
...
//	   fprintf(stderr, "** DBG: snrV=%2.1f\n",snrV);

// OLD	   float sum2 = myReader. get_sLevel ();
// OLD	   snr	= 0.9 * snr + 0.1 * 20 * log10 ((sum2 + 0.005) / sum + 0.005);

/* To calculate the SNR in DAB, the following formula can be used:
   SNR = 10 * log10(Ps / Pn)
   Where: SNR: Signal-to-noise ratio (dB)
          Ps:  Signal power (in watts or dBm)
          Pn:  Noise power (in watts or dBm)
   The signal power (Ps) is usually measured at the output of the receiver's front-end, while
   the noise power (Pn) is measured in the same frequency band as the DAB signal, but without
   the presence of the DAB signal. The difference between the two measurements gives the SNR
   value in dB. */

           ccc ++;
           if (ccc >= 50) { //10
              snr = 0.85 * snr +
                    0.15 * 20 * log10 ((myReader. get_sLevel() + 0.005) / (sum + 0.005)); 
// OLD                   0.1 * 20 * log10 ((myReader. get_sLevel() + 0.005) / (sum + 0.005)); 
	      fprintf(stderr, "** DBG: snr(%d)=%2.1f\n",ccc,snr);
	      ccc = 0;
           }
...
//	   fprintf(stderr,"fineoff=%f ",fineOffset);
...
void    dabProcessor::dataforAudioService	(const std::string &s,audiodata *dd) {
	std::string ss = s;
	fprintf( stderr, "DBG: fic->dataforAudioService1\n");
        my_ficHandler. dataforAudioService (ss, dd, 0);
}

void    dabProcessor::dataforAudioService	(const std::string &s,audiodata *d, int16_t c) {
	std::string ss = s;
	fprintf( stderr, "DBG: fic->dataforAudioService2\n");
        my_ficHandler. dataforAudioService (ss, d, c);
}

void    dabProcessor::dataforDataService	(const std::string &s,packetdata *d, int16_t c) {
	std::string ss = s;
