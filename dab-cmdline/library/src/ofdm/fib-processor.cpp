...
const char *getFIGtype (int8_t PacketType) {
        switch (PacketType) {
           case 0:     return "MCI/SI   "; 
           case 1:     return "Labels/SI"; 
           case 2:     return "Labels/SI";
...
uint8_t	extension;
	...
	while (processedBytes  < 30) {
	   ...
	   switch (FIGtype) {
	      case 0:	// MCI and part of the SI
//		 fprintf(stderr,"** DBG: FIG%d/%d(%s)[%d]\n",FIGtype,extension,getFIGtype(FIGtype),FIGlength);
	         process_FIG0 (d);	
	         break;

	      case 1:	// Labels, etc. (part of the SI)
//		 fprintf(stderr,"** DBG: FIG%d/%d(%s)[%d]\n",FIGtype,extension,getFIGtype(FIGtype),FIGlength);
	         process_FIG1 (d);
	         break;

	      case 7:	// Reserverd (except for length 31) - End marker acc. to 5.2.2.0 of EN 300 401
//	         process_FIG7 (d);
	         break;

	      default:
//	         fprintf (stderr, "*!* DBG Found FIG%d *!*\n", FIGtype);
		 fprintf(stderr,"*!* DBG: FOUND FIG%d(%s)[%d] = 0x",FIGtype,getFIGtype(FIGtype),FIGlength-processedBytes);
		 for (i = 0; i < FIGlength-processedBytes; i++) {
		 	fprintf(stderr,"%02x",getBits_8(d,i*8));
		 } fprintf(stderr,"\n");
	         break;
	   }
//
...
//	a p rather than a d
	   processedBytes += getBits_5 (d, 3) + 1;	//	   processedBytes += getBits (p, 3, 5) + 1;
	   d = p + processedBytes * 8;
     	   // NOTE FIG0(MCI/SI   )[29/30] -> immer auffällig!
//	   if (processedBytes > 29) {
//		fprintf(stderr,"** DBG: FIG%d(%s)[%d/%d]\n",FIGtype,getFIGtype(FIGtype),FIGlength,processedBytes);
//	   }
	}
	fibLocker. unlock ();
}
...
//        fprintf(stderr,"** DBG: FIG%d(%s)[t=%02d]\n",0,getFIGtype(0),extension);

	switch (extension) {
	   case 0:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Ensemble information\n",extension);
	      FIG0Extension0 (d);
	      break;

	   case 1:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Sub-channel organization\n",extension);
	      FIG0Extension1 (d);
	      break;

	   case 2:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service organization\n",extension);
	      FIG0Extension2 (d);
	      break;

	   case 3:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service component in packet mode\n",extension);
	      FIG0Extension3 (d);
	      break;

	   case 4:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service component with CA in stream mode\n",extension);
	      FIG0Extension4 (d);
	      break;

	   case 5:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service component language\n",extension);
	      FIG0Extension5 (d);
	      break;

	   case 6:
	      FIG0Extension6 (d);
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service linking information\n",extension);
	      break;

	   case 7:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Configuration information\n",extension);
	      FIG0Extension7 (d);
	      break;

	   case 8:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service component global information\n",extension);
	      FIG0Extension8 (d);
	      break;

	   case 9:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Country, LTO & international table\n",extension);
	      FIG0Extension9 (d);
	      break;

	   case 10:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Date & time\n",extension);
	      FIG0Extension10 (d);
	      break;

	   case 11:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Unknown\n",extension);
	      FIG0Extension11 (d);
	      break;
...
//	      fprintf(stderr,"** DBG: FIG0/%2d: Unknown\n",extension);
	   ...
	   case 13:
//	      fprintf(stderr,"** DBG: FIG0/%2d: User Application information\n",extension);
	      FIG0Extension13 (d);
	      break;

	   case 14:
//	      fprintf(stderr,"** DBG: FIG0/%2d: FEC sub-channel organization\n",extension);
	      FIG0Extension14 (d);
	      break;
...
//	      fprintf(stderr,"** DBG: FIG0/%2d: Unknown\n",extension);
	   ...
	   case 17:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Programme Type (PTy)\n",extension);
	      FIG0Extension17 (d);
	      break;

	   case 18:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Announcement support\n",extension);
	      FIG0Extension18 (d);
	      break;

	   case 19:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Announcement switching\n",extension);
	      FIG0Extension19 (d);
	      break;

	   case 20:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Service component information\n",extension);
	      FIG0Extension20 (d);
	      break;

	   case 21:
//	      fprintf(stderr,"** DBG: FIG0/%2d: Frequency information (FI)\n",extension);
	      FIG0Extension21 (d);
	      break;
...
//	      fprintf(stderr,"** DBG: FIG0/%2d: Unknown\n",extension);
	   ...
	   case 24:
//	      fprintf(stderr,"** DBG: FIG0/%2d: OE services\n",extension);
	      FIG0Extension24 (d);
	      break;

	   case 25:
//	      fprintf(stderr,"** DBG: FIG0/%2d: OE announcement support\n",extension);
	      FIG0Extension25 (d);
	      break;

	   case 26:
//	      fprintf(stderr,"** DBG: FIG0/%2d: OE announcement switching\n",extension);
	      FIG0Extension26 (d);
	      break;
...
uint8_t i;
	...
	if (getBits_1 (d, bitOffset + 16) == 0) {	// short form
	   ...
	   if (option == 001) {		// B Level protection
	      ...
	      fprintf(stderr,"*!* FIG0/1 Unknown option=%d\n",option);
	   }
/*           if (SubChId == 32) {
              fprintf(stderr, "** DBG SC=%02d ProtLevel=%d\n",SubChId,protLevel);
           }*/
	   bitOffset += 32;
	}
/*	if (SubChId == 32) {
//	   fprintf(stderr, "** DBG: Got %d bytes for subchannel %d [%d] = 0x",bitOffset/8,SubChId,subChannels [SubChId]. Length);
// TPEG       ** DBG: Got 10 bytes for subchannel 32 [6] = 0x1d0185da880681d48806
// PP-RTK-AdV ** DBG: Got 10 bytes for subchannel 32 [6] = 0x1d0185da880681d48806

	   for (i = 0; i < bitOffset/8; i++) {
//	   	fprintf(stderr,"%s",std::to_string(getBits_8(d,i*8)));
	   	fprintf(stderr,"%02x",getBits_8(d,i*8));
	   } fprintf(stderr,"\n");
	}*/
...
int16_t		numberofComponents;
	...
	for (i = 0; i < numberofComponents; i ++) {
	   ...
	   if (TMid == 3) { // MSC packet data
	      uint16_t SCId	= getBits (d, lOffset + 2, 12);
	      uint8_t PS_flag	= getBits_1 (d, lOffset + 14);
	      uint8_t CA_flag	= getBits_1 (d, lOffset + 15);
	      bind_packetService (TMid, SId, i, SCId, PS_flag, CA_flag);
//	      fprintf(stderr,"** FIG0/2 pd=%d SId=%d SCId=%d PS=%d CA=%d\n",pd,SId, SCId, PS_flag, CA_flag);
           }
	   else
	      {
	      	fprintf(stderr,"*!* DBG Unhandeld input von FIG0/2 TMID=%d\n",TMid);
	      }		// for now
	   lOffset += 16;
	}
...
uint16_t CAOrg;
uint8_t Rfa		= getBits(d, used*8+12,3);
uint8_t Rfu		= getBits_1(d, used *8 +17);
...
//      with it
	...
	packetComp	-> DGflag	= DGflag;
        ...
        fprintf(stderr,"** FIG0/3 SC=%d DSCTy=%d DG=%d pAdr=%d Rfa=%d rfu=%d\n",SubChId,DSCTy,DGflag,packetAddress,Rfa,Rfu);
        return used;
}
//
...
//        fprintf(stderr,"FIG0/4: Rfa=\t%D, Rfu=\t%d, SudChId=\t%02X, CAOrg=\t%04X\n", Rfa, Rfu, SubChId, CAOrg);
        fprintf(stderr,"FIG0/4: Rfa=%d Rfu=%d ",Rfa,Rfu); 
        fprintf(stderr,"SudChId=%02X CAOrg=%04X\n", SubChId, CAOrg);
...
//	take care of different time zones
	bool	change = false;
	for (int i = 0; i < 5; i ++) {
	   if (theTime [i] != static_cast<uint32_t>(dateTime [i]))
	      change = true;
...
//	FIG type 0 flags (5.2.2.1) are used as follows C/N -> MCI // OE flag->Rfu // P/D flag -> Rfu
void	fib_processor::FIG0Extension14 (uint8_t *d) {
int16_t	Length	= getBits_5 (d, 3);	// in Bytes
int16_t	used	= 2;			// in Bytes
uint8_t mci 	= getBits_1 (d, 8);	// FIG Type 0 C/N
uint8_t rfa	= getBits_1 (d, 9);	// FIG Type 0 OE Flag
uint8_t rfb	= getBits_1 (d,10);	// FIG Type 0 P/D Flag
	/* 6.2
	   6.3
	   6.4
	   8.1*/
//	fprintf(stderr,"** DBG FIG0/14 mci=%d Rfu=%d Rfu=%d\n",mci,rfa,rfb);

	while (used < Length) {
	   int16_t SubChId	= getBits_6 (d, used * 8);
	   uint8_t FEC_scheme	= getBits_2 (d, used * 8 + 6);
	   uint8_t extf		= getBits   (d, used * 8 + 8,8);
	   used = used + 1;
	   if (subChannels [SubChId]. inUse)
	      subChannels [SubChId]. FEC_scheme = FEC_scheme;
//	if (SubChId == 32) {
//	   fprintf(stderr,"** FIG0/14: mci=%d Rfa=%d rfb=%d SubChId=%d FECsch=%d EF=%02X\n",mci,rfa,rfb,SubChId,FEC_scheme,extf);
	   // ** FIG0/14: mci=0 Rfa=0 rfb=0 SubChId=32 used=5 length=5 FECsch=1 EF=85
//	}
	}
}
...
//	      fprintf(stderr,"** DBG: FIG1/%2d: Ensemble label\n",extension);
	      SId	= getBits (d, 16, 16);
	      offset	= 32;
...
//	      fprintf(stderr,"** DBG: FIG1/%2d: Programme service label\n",extension);
	      SId	= getBits (d, 16, 16);
	      offset	= 32;
...
//	         fprintf (stderr, "FIG1/1: SId = %4x\t%s\n", SId, label);
	   ...
	   case 3:
	      // region label
//	      fprintf(stderr,"** DBG: FIG1/%2d: Region label\n",extension);
...
//	      fprintf(stderr,"** DBG: FIG1/%2d: Service component label\n",extension);
...
//              fprintf(stderr,"** DBG: FIG1/%2d: Data service label\n",extension);
...
//	      fprintf(stderr,"** DBG: FIG1/%2d: X-PAD User Application label\n",extension);
	      pd_flag	= getLBits (d, 16, 1);
	      SCidS	= getLBits (d, 20, 4);
...
//	      fprintf (stderr, "FIG1/%d: not handled now\n", extension);
	      fprintf (stderr, "FIG1/%d passed by\n", extension);
	      break;
	}
	(void)SCidS;
	(void)XPAD_aid;
...
// FIG 7 - RESERVED
// ** FIG7: FF 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 
void    fib_processor::process_FIG7 (uint8_t *d) {
uint8_t		offset	= 8;
int16_t		i;

	fprintf(stderr,"** FIG7: ");
	for (i = 0; i < 31; i ++) { 
		fprintf(stderr,"%02X", getBits_8(d, offset+8*i)); //fgd);
	}
	fprintf(stderr,"\n");
	(void)d;
}
...
serviceId *selectedService;
	...
	if (selectedService == nullptr) {
	   fprintf(stderr,"*!* DBG exit as no service was found *!*\n");
	   fibLocker. unlock ();
	   return;
	} else {
	   fprintf(stderr,"*** DBG Selected Service = %s\n",s.c_str());
	}
