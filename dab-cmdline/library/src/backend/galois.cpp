...
/*	uint16_t symsize = 8;   // default symsize
	uint16_t gfpoly = 0x11d; // default generator polynomial for GF(2^8)*/

	// call the parameterized constructor to initialize the other members
	init(symsize, gfpoly);
}
/*		galois::galois (uint16_t symsize, uint16_t gfpoly) {
		galois(symsize, gfpoly); }*/

           void galois::init(uint16_t symsize, uint16_t gfpoly) {
uint16_t sr;
uint16_t i;
