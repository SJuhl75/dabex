...
/*	ETSI TS 102 563 ... Annex C Synchronizing to the audio super frame structure
	1) At least 5 DAB frames are stored one after the other into the audio super frame buffer.
	   This step takes d × 24 ms, where d is the number of DAB frames that are added to the audio
	   super frame buffer. d is at least 5, so this step takes at least 120 ms.
	2) A first test is used to check if the first DAB frame in the audio super frame buffer might
	   be the beginning of an audio super frame. If that is not the case, go to step 4.
	3) It is checked whether the first five DAB frames are a valid audio super frame (i.e. the
	   first DAB frame contains a valid audio super frame header). The receiver does the RS
	   decoding to correct all reception errors (if there were any) and then checks whether the
	   header checksum is correct; for this test, the Fire code is used for error detection only.
	   If that is the case, then the first valid audio super frame was received. Synchronization
	   is completed.
        4) If the first DAB frame of the audio super frame buffer is not a valid beginning of an audio
           super frame, then the first DAB frame is removed from the audio super frame buffer and the
           next received DAB frame is added to the end of the audio super frame buffer. DAB frames are
           received every 24 ms. We then restart at step 2. */
...
//	per Byte, nbits is the number of Bits (i.e. containing bytes)
	for (i = 0; i < nbits / 8; i ++) {	// in bytes
	   temp = 0;
	   for (j = 0; j < 8; j ++) 
	      temp = (temp << 1) | (V [i * 8 + j] & 01); // V [i * 8 + j] & 01 -> Bitpuffer
	   frameBytes [blockFillIndex * nbits / 8 + i] = temp;
	} // = 312 Bytes x 5 = 1560 Bytes
//
	blocksInBuffer ++;
...
//	we take the last five blocks to look at
	if (blocksInBuffer >= 5) {
//	   fprintf(stderr,"**** DBG mp4:addtoFrame %d bytes\n", temp);
	   if (++frameCount >= 50) {
	      frameCount = 0;
...
// NOTE see ETSI TS 102 563 section 6.2
//  	fprintf(stderr,"**** DBG: processSuperframe with %d of %ld bytes.\n",base,sizeof(frameBytes));
	for (j = 0; j < RSDims; j ++) {
	   int16_t ler	= 0;
	   for (k = 0; k < 120; k ++) 
	      rsIn [k] = frameBytes [(base + j + k * RSDims) % (RSDims * 120)];
//
//	   fprintf(stderr,"*** DBG: RS In=%hhn, Out=%hhn, ler=",rsIn,rsOut );
...
// NOTE Daten sind nach RS-Decoding im outVector	
...
//	      fprintf(stderr,"**** DBG: mp4:aacDecoder %d bytes -> aacDecoder\n",aac_frame_length);
...
uint8_t theAudioUnit [2 * 960 + 10];	// sure, large enough
	...
	fprintf(stderr,"**** DBG: mp4:handle_aacFrame %d bytes -> aacDecoder\n",frame_length);
