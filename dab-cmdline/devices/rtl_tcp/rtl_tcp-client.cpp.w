...
void print_buffer_as_hex(const uint8_t* buffer, size_t size) {
...
//	reference: https://hz.tools/rtl_tcp/
	...
	sendCommand (0x05, 2); //ppm); // berechnet 25.04.2023
	sendCommand (0x0e, 1); // bias tee high(1) low(0)
//
...
void	rtl_tcp_client:: run (void) {
	...
	const char* tuner[7]  = { "UNKNOWN", "E4000", "FC0012", "FC0013", "FC2580", "R820T", "R820D" };
	...
	while (running. load ()) {
	      ...
	      running. store (false);
           ...
           if (first_time) {	// equal to first_time != 0
           	first_time = 0;
	        fprintf(stderr, "** RTL_TCP Client\n");
                print_buffer_as_hex(buffer, res);
