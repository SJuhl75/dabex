...
//	Print a resolved address of server (the first IP of the host)
	//DEBUG_PRINT
	fprintf(stderr,"server address = %d.%d.%d.%d, port %d \n",
...
//	and set the parameters right
	...
	sendCommand (0x0e, true); // Set Bias Tee ON
	sendCommand (0x01, vfoFrequency);
	sendCommand (0x03, autogain);
	// sendCommand (0x08, autogain); Software "Gain"
	//DEBUG_PRINT 
	fprintf(stderr,"setting the gain to %d\n", gain);
	sendCommand (0x04, 10 * gain);
	//sendCommand (0x0d, 15); // 3 -> 16.2 5-> 16.5 does not 
...
//	                        (float (tempBuffer [2 * i + 1] - 128)) / 128.0);
	                        (float (tempBuffer [2 * i] - 127.4f)) / 128.0f,
	                        (float (tempBuffer [2 * i + 1] - 127.4f)) / 128.0f);
	return amount / 2;
}

/* NOTE https://github.com/AlexandreRouma/SDRPlusPlus/issues/1260
